...
class AntibioticsSpacyStrategy(AbstractStrategy):
    ...
    def extract_medicine_list(cls, note_text):
        categories_list = ['history_of_present_illness','observation_and_plan','medications']
        nlp = cls.get_nlp()
        ...
        for sec in doc._.sections:
            ...
            if sec.category in categories_list:
                section_text = doc[sec.body_start:sec.body_end].text
                print(f"Section text is {section_text}")
                return section_text.strip(":?-_ \xa0\n")
                e
        return None
